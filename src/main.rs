    ...
    if let Some(ref matches) = matches.subcommand_matches("rename") {
        ...
        if let Some(values) = matches.values_of("rename") {
            for value in values {
                ...
                let input_file =
                    InputFile::new(value, matches.value_of("move").map(|f| PathBuf::from(f)))?;
                let file_parts = input_file.input_file_parts.clone();
                ...
                if let Some(name_result) = name_result {
                    let input_file = input_file.set_output_file_name(name_result);
                    println!(
                        "Rename {} to {}",
                        input_file.input_path.display(),
                        input_file.output_path()?.display()
                    );
                    if !dry_run {
